#! /bin/bash
# using shebang and .sh to make the plebs happy, 
# neither are necessary and are redundant and dumb when already using a bash shell

### SCRIPT DESCRIPTION ###
# This script makes a file called start_minecraft_server.sh to allow easy starting of the server.
# It uses spigot to produce a minecraft server jar which should be called spigot-*.jar where * is some string.
# Once the script sucessfully makes start_minecraft_server.sh and spigot-*.jar, refer to start_minecraft_server.sh to edit server memory allocation
# If you want to use a new version of the spigot build (e.g. spigot jar doesnt support the most current minecraft version), then delete BuildTools.jar, spigot-*.jar, and start_minecraft_server.sh first
...
(echo "Check that https://hub.spigotmc.org/jenkins/job/BuildTools/lastSuccessfulBuild/artifact/target/BuildTools.jar exists. If not look on https://hub.spigotmc.org for new jar file" && \
exit 1))
# build server jar file with specified version and use 8GB while building
...
[ -f start_minecraft_server.sh ] && exit 1
touch start_minecraft_server.sh
echo "#! /bin/bash" >> start_minecraft_server.sh
echo "java -Xmx""$run_gb""G -Xms""$run_gb""G -jar spigot-""$minecraft_version"".jar nogui" >> start_minecraft_server.sh
